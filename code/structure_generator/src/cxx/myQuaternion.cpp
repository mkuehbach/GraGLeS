...
myQuaternion* myQuaternion::randomRotationSO3Shoemake( randomClass& thernd )
{
	...
	//K. Shoemake, Graphic Gems III (editor D. Kirk) CalTech pp124-134
	double X0 = thernd.MersenneTwister();
	double X1 = thernd.MersenneTwister();
	double X2 = thernd.MersenneTwister();
...
myQuaternion* myQuaternion::DisorientedFromReference(
	...
	while (foundvalid == false) {
		...
		cand = this->randomRotationSO3Shoemake( r ); //MK::allocates heap and returns pointer to candidate, instead it could work on the stack probably much more efficient...

		//apply the random rotation candidate on myself, apply crystal symmetry
		...
		if (prob <= coinFlip) {
			foundvalid = true;
			break; //and as we accept this orientation we do free the heap memory object
		}

		//cand was unsuccessful, so we need to find another one, but first we must free the heap memory object
		delete cand;
		cand = NULL;
	}

	//apply the candidate as a random rotation to get a properly disoriented orientation to the parent grain
